export const W = 800;
export const H = 600;
export const CX = W / 2;
export const CY = H / 2;
export const PLAYER_RADIUS = 10;
export const ENEMY_RADIUS = 5;
export const BULLET_RADIUS = 1.5;

export const BULLET_SPEED = -400;
export const BULLET_DISPERSION = 1.5;

export const ENEMY_START_DIST = 250;
export const FPS = 60;

// balacing constants

export const BULLET_RATE_UP = 0.05;
export const BULLET_DAMAGE_UP = 0.1;
export const BULLET_RADIUS_UP = 0.1;

// enemies constants

export const ENEMY_COUNT_START = 10;
export const ENEMY_RATE_START = 1;
export const ENEMY_HP_START = 10;
export const ENEMY_SPEED_START = 20;

export const ENEMY_COUNT_UP = 0.01;
export const ENEMY_RATE_UP = 0.005;
export const ENEMY_HP_UP = 0.05;
export const ENEMY_SPEED_UP = 0.004;